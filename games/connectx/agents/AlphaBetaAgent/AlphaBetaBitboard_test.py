...
def test_first_move_is_center_depth(observation, configuration, search_max_depth):
    action = AlphaBetaBitboard.agent(search_max_depth=search_max_depth)(observation, configuration)
...
def test_can_play_game_against_self():
    ...
    env.run([ AlphaBetaBitboard.agent(), AlphaBetaBitboard.agent() ])
...
def test_can_win_against_kaggle(position, opponent):
    ...
    if position == "player1":
        env.run([AlphaBetaBitboard.agent(), opponent])
        assert env.state[0].reward == 1
        assert env.state[1].reward == -1
    else:
        env.run([opponent, AlphaBetaBitboard.agent()])
        assert env.state[1].reward == 1
        assert env.state[0].reward == -1
